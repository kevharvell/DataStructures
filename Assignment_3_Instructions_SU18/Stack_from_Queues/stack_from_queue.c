...
void listQueueInit(struct listQueue *q) {
       ...
       q->firstLink = q->lastLink = (struct link*) malloc(sizeof(struct link));
       assert(q->firstLink != 0 && q->lastLink != 0);
       q->firstLink->next = q->lastLink->next = 0;
       q->size = 0;
}
...
void listQueueAddBack(struct listQueue *q, TYPE e) {
       // FIXME: you must write this
	q->lastLink->next = (struct link*) malloc(sizeof(struct link));
	assert(q->lastLink->next != 0);
